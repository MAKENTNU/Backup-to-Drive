...
parser.add_argument(
    "-i",
    "--folder-id",
    default=settings.DRIVE_BACKUP_FOLDER_ID,
    type=str,
...
try:
    creds = service_account.Credentials.from_service_account_file(
        settings.CREDENTIALS_FILENAME, scopes=settings.GOOGLE_API_SCOPES
    )
except (FileNotFoundError, JSONDecodeError, ValueError) as e:
    logger.exception(
        f"Error while parsing credentials from '{settings.CREDENTIALS_FILENAME}'.",
        exc_info=e,
    )
    logger.error(
        "Please use the following guide on creating service account credentials/keys:"
...
# --- Upload the file ---
logger.info(
    f"Uploading '{original_name}' to Drive (folder ID: '{drive_folder_id}') with"
    f" the filename '{backup_name}'"
)
# Code based on
...
for i in range(settings.NUM_UPLOAD_RETRIES):
    try:
        ...
        logger.info(
            f"Backup finished! View the file at {uploaded_file.get('webViewLink')}"
        )
        break
